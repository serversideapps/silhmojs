...
class Articles @Inject() (
  ...
  val index = Action.async { implicit request =>
    ...
    found.flatMap(_.collect[List](
      MONGO_COLLECT_MAX, Cursor.FailOnError[List[Article]]()
    )).map { articles =>
      Ok(views.html.articles(None, articles, activeSort))
    }.recover {
      case e =>
        e.printStackTrace()
  ...
  def showCreationForm = Action { implicit request =>
    ...
    Ok(views.html.editArticle(None, None, Article.form, None))
  }
      ...
      result <- maybeArticle.map { article =>
        ...
        fs.find[JsObject, JSONReadFile](
          Json.obj("article" -> article.id.get)
            ...
            MONGO_COLLECT_MAX, Cursor.FailOnError[List[reactivemongo.api.gridfs.ReadFile[reactivemongo.play.json.JSONSerializationPack.type, play.api.libs.json.JsString]]]()
          ).map { files =>
  ...
  def create = Action.async { implicit request =>
    ...
    Article.form.bindFromRequest.fold(
      errors => Future.successful(
        Ok(views.html.editArticle(None, None, errors, None))
      ),
  ...
  def edit(id: String) = Action.async { implicit request =>
    ...
    Article.form.bindFromRequest.fold(
      errors => Future.successful(
        Ok(views.html.editArticle(None, Some(id), errors, None))
      ),
  ...
  def delete(id: String) = Action.async {
    ...
    } yield Redirect(routes.Articles.index)).recover { case _ => InternalServerError }
  }
  ...
  def saveAttachment(id: String) = {
    ...
    Action.async(gridFSBodyParser(fs)) { request =>
      ...
      val futureUpdate = for {
        ...
        updateResult <- {
          println("file id " + file.id); fs.files.update(
  ...
  def removeAttachment(ids: String) = Action.async {
    val idparts = ids.split("_")
    val id = idparts(0)
    val ida = idparts(1)
    gridFS.flatMap(_.remove(Json toJson id).map(_ => Redirect(routes.Articles.showEditForm(ida))).
