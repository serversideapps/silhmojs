...
case class FlickPiece(
    // to be serialized to keep track of the position
    var id: String = "",
    var piece: TPiece = NO_PIECE,
    var x: Double = 0.0,
    var y: Double = 0.0,
    var rot: Double = 0.0,
    // used for simulation only
    var vx: Double = 0.0, // speed in x direction
    var vy: Double = 0.0 // speed in y direction
) {

  def m = 1.0 // mass, can depend on piece value

  def serialize: String = s"""$id $piece $x $y $rot"""

  def deserialize(rep: String): FlickPiece = {
    val parts = rep.split(" ")
    id = parts(0)
    piece = parts(1).toInt
    x = parts(2).toDouble
    y = parts(3).toDouble
    rot = parts(4).toDouble
    this
  }
}

case class FlickPieces(
    var pieces: Map[String, FlickPiece] = Map[String, FlickPiece]()
) {
  def serialize: String = (for ((k, v) <- pieces) yield v.serialize).mkString("  ")

  def deserialize(rep: String): FlickPieces = {
    pieces = Map[String, FlickPiece]()
    val parts = rep.split("  ")
    for (part <- parts) {
      val p = FlickPiece().deserialize(part)
      pieces += (p.id -> p)
    }
    this
  }
...
object board {
  ...
  def DEFAULT_VARIANT = "Four Player"
...
class board(setvariant: String = board.DEFAULT_VARIANT) {

  var variant = setvariant

  var b4: board4 = board4()
  ...
  def getturn = turn
  def turni: Int = {
    if (IS_FOUR_PLAYER) return b4.turn4
    if (IS_FLICK) {
      if (flickturn == "white") return 0
      return 1
    }
    if (turn == WHITE) return 0
    1
  }
  def numplayers = if (!IS_FOUR_PLAYER) 2 else board4.NUM_PLAYERS
  ...
  def set_from_fen_flick(fen: String) {
    try {
      val parts = fen.split("   ")
      turn = parts(0).toInt
      flickpieces = FlickPieces().deserialize(parts(1))
      flickid = parts(2)
      flickvx = parts(3).toDouble
      flickvy = parts(4).toDouble
      flickresult = parts(5)
      flickturn = parts(6)
  ...
  def set_from_fen(fen: String) {
    ...
    if (IS_FOUR_PLAYER) {
      b4.set_from_fen4(fen)
      return
    }

    if (IS_FLICK) {
      set_from_fen_flick(fen)
      return
    }
  ...
  def report_fen_flick: String = {
    turn + "   " +
      flickpieces.serialize + "   " +
      flickid + "   " +
      flickvx + "   " +
      flickvy + "   " +
      flickresult + "   " +
  ...
  def report_fen: String =
    {
      if (IS_FOUR_PLAYER) return b4.report_fen4

      if (IS_FLICK) return report_fen_flick
...
""".stripMargin
  ...
  def makeAlgebMove(algeb: String): Boolean = {
    if (IS_FOUR_PLAYER) {
      return b4.makeAlgebMove(algeb)
    }

    makeMove(move(fromalgeb = algeb))
    true
  }
  ...
  def isAlgebLegal(algeb: String, test: Boolean = false): Boolean =
    {
      if (IS_FOUR_PLAYER) return b4.isAlgebLegal(algeb)

      if (test) println("testing " + algeb + " at " + report_fen)

      val m = move(fromalgeb = algeb)

      if (test) println("move is " + m)
  ...
  def isMoveLegal(m: move, test: Boolean = false): Boolean =
    {
      if (test) println("checking square validity")
      ...
      if (test) println("trying to set current ptr")
      ...
      if (IS_RACING_KINGS) {
        ...
        for (i <- -1 to 1) {
          ...
          if (fileOk(tf)) {
            ...
            if ((!isInCheckSq(testsq, allow_global = false))
              &&
              (isSqFree(testsq))) return false
          }
        }
  ...
  def isAlgebPromotion(algeb: String): Boolean = {
    val m = move(fromalgeb = algeb)
    val rm = toRichMove(m)
    if (rm.pawn_promotion && isAlgebLegal(algeb + "q")) return true
    false
  }
      ...
      while (currentSq < BOARD_AREA) {
        ...
        while ((!move_table(currentPtr).end_piece) && (!piece_done)) {
          ...
          if (to != NO_SQUARE) {
            ...
            var atomic_knight_ok = true

            if (IS_ATOMIC && fullmove_number <= ATOMIC_NO_KNIGHT) {
              if (typeOf(from_piece) == KNIGHT) atomic_knight_ok = false
            }

            if ((colorOf(from_piece) == turn) && (atomic_knight_ok)) {
  ...
  var move_list_algebs = List[String]()
  ...
  def genMoveList {
    ...
    move_list_algebs = move_list.map(m => m.toAlgeb).toList
  }
  ...
  def reset_flick {
    val b = new board("Standard")
    b.reset

    flickpieces = FlickPieces()

    for (i <- 0 to (BOARD_AREA - 1)) {
      val file = i % BOARD_SIZE
      val rank = (i - file) / BOARD_SIZE
  ...
  def toRawSan(m: move): String =
    {
      ...
      if (rm.pawn_move || (rm.king_move && (variant != "Antichess"))) {
        return rm.toSan()
      }
  ...
  def NUM_FILES: Int = {
    if (!IS_FOUR_PLAYER) return square.BOARD_SIZE
    square4.BOARD_SIZE4
  }
  ...
  def LAST_FILE = NUM_FILES - 1

  def LAST_RANK = NUM_RANKS - 1

  def PROMOTION_PIECES: List[Char] = {
    if (!IS_ANTICHESS) return List('q', 'r', 'b', 'n')
    return List('k', 'q', 'r', 'b', 'n')
  }
